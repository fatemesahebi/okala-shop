...
const MobileHeaderCategory = ({
  ...
  const categorizeResultsData = menuData.filter(
    (item) => item.mainTitle === categoryName
  );
  let categoryTags = categorizeResultsData
    ? [
        categorizeResultsData?.[0]?.items1?.[0]?.title,
        categorizeResultsData?.[0]?.items1?.[1]?.title,
      ]
    : ["", ""];
  return (
    <Box>
      ...
      >
        <Toolbar
          variant="dense"
          sx={{ height: "137px", alignItems: "flex-start" }}
        >
          <Box
