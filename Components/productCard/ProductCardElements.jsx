...
const SingleProduct = ({ product }) => {
  ...
  let shoppingCardId = useSelector((state) =>
    state.cart.items.findIndex((item) => item.id === product.id) > -1
      ? state.cart.items.findIndex((item) => item.id === product.id)
      : -1
  );
  let shoppinCardCount = useSelector((state) =>
    shoppingCardId === -1 ? 0 : state.cart.items[shoppingCardId].count
  );
          ...
          <StyledCard>
            <StyledCardMedia>
              ...
              {shoppinCardCount > 1 && (
                <div>
                  <StyledShoppingCount>
                    {PN.convertEnToPe(shoppinCardCount)}
                  </StyledShoppingCount>
                  <StyledMinusIcon>
                    <RemoveIcon
                      onClick={() => {
              ...
              <StyledImage
                onClick={() => {
                  router.push(`/products/${product.id}/${productUrl}`);
                }}
            ...
            <StyledCardContent>
              <Link
                href={`/products/${product.id}/${productUrl}`}
                underline="none"
                color="inherit"
              >
                <StyledProductName variant="paragraph">
                  {product.productName}
                </StyledProductName>
                <StyledPrice
                  style={{
                ...
                <div>
                  <Grid container>
                    <Grid item>
                      <StyledOffPercent
                        style={{
                          visibility:
                            product.offPercent === 0 ? "hidden" : "visible",
                        }}
                        variant="paragraph"
                      >
                        {PN.convertEnToPe(
                          PN.sliceNumber(`${product.offPercent}`)
                        )}{" "}
                        %
                      </StyledOffPercent>
                    </Grid>
                  ...
                  <Grid item>
                    <StyledPriceOffer variant="paragraph">
                      {PN.convertEnToPe(
                        PN.sliceNumber(`${product.priceOffer}`)
                      )}{" "}
                      ریال
                    </StyledPriceOffer>
                  </Grid>
